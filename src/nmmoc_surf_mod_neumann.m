...
% difference method
%%
function [UU,CC,GG,ocut,wcut,ROIP] = nmmoc_surf_mod_neumann(u,v,S,C,G,miua,para,sigma,c0,g0,src)
...
lambda_a = compmob(Q,miua,C,sor,swr,1,1);
lambda_o = compmob(Q,miua,C,sor,swr,0,1);
...
lambda_a = compmob(Qmod,miua,C,sor,swr,1,1);
lambda_o = compmob(Qmod,miua,C,sor,swr,0,1);
lambda = lambda_a + lambda_o;
f = lambda_a./lambda;
            ...
            if j==i
                ...
                elseif iter == (m)*(n-1)+1   %topmost row of grid
                    ...
                    elseif i == m            % right most column of grid
                        ...
                        DD(i) =(Qmod(cnt+1,i)/dtcal(cnt+1,i)) - f_c(cnt+1,i)*(u(cnt+1,i)*(C(cnt+1,i+1)-C(cnt+1,i-1))...
...
%new redefined coordinates for concentration equation
[xmod2,ymod2] =  eval_Xsurf_neumann(x,y,Q,Qnew,G,f,f_s,D,pc_s,pc_g,u,v,dtcal,para,3);
