...
% concentrations C00 and a grid size 24X24
clc
clear all;
clear global all;
format long
...
for counter = 1:3      
    ...
    c0=C00(counter);
    [UU,CC,~]=s0c0(para,phi_test,s0,c0,g0,0);
...
%         end
        ...
        if rem(tcal,500)==0
            tcalc=tcal/10;
            export_fig(sprintf('./Run10/ST%dc%g.pdf', tcalc, c0));
        ...
        if rem(tcal,500)==0
            export_fig(sprintf('./Run10/CT%dc%g.pdf', tcalc, c0));
