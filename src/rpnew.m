function out=rpnew(u,v,U,miuo,miuw,alpha,dx,dy,dt,KK)
...
%     AAA=zeros(size(Q,1)*size(Q,2));
    while(iter<=size(Q,1)*size(Q,2))
            ...
            for j=1:1:size(Q,2) %% j = y direction discretization and columns ie x direction in algorithm
                if(j==i)
                    if(iter>size(Q,2))
                        ...
                        AA(i,j,1)   = - (Dw_t(cnt+1,j+1)+Dw_t(cnt+2,j+1))/(2*dy*dy);
                    ...
                    DD(j,1)     = (1/dt)*Q(cnt+1,j,1)- (1/(2*dx))*(Fw_ut(cnt+2,j+2)-Fw_ut(cnt+2,j))-(1/(2*dy))*(Fw_vt(cnt+3,j+1)-Fw_vt(cnt+1,j+1));
                    DD(j,2)     = (1/dt)*Q(cnt+1,j,2)- (1/(2*dx))*(Fc_ut(cnt+2,j+2)-Fc_ut(cnt+2,j))-(1/(2*dy))*(Fc_vt(cnt+3,j+1)-Fc_vt(cnt+1,j+1));
                end
            end
        ...
        elseif(rem(iter-1,size(Q,2))==0 && (iter-1)/size(Q,2)<size(Q,1)-1)
           AAA(iter:iter+size(Q,1)-1,:) = [zeros(size(Q,1),(iter-1)-size(Q,2)),AA,BB,CC,zeros(size(Q,1),size(Q,1)*size(Q,2)-3*size(Q,2)-(iter-1)+size(Q,2))];
    ...
    Q(:,:,1)=reshape(bicgstab(AAA,DDD(:,1),1*10^(-4),30,[],[]),size(Q,1),size(Q,2))
    %Q(:,:,2)=reshape(bicgstab(AAA,DDD(:,2)),size(Q,1),size(Q,2));
    pause;
end
