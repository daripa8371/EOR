            ...
            if j==i
                ...
                elseif iter == (m)*(n-1)+1
                    ...
                    elseif i == m
                        ...
                        DD(i) = (1/dt)*Cmod(cnt+1,i);
                        AA(j,i)   =  -D(cnt+1,i)/Qnew(cnt+1,i)*(Qnew(cnt+2,i)-Qnew(cnt,i))/(4*dy*dy);
                        CC(j,i)   =  D(cnt+1,i)/Qnew(cnt+1,i)*(Qnew(cnt+2,i)-Qnew(cnt,i))/(4*dy*dy);
                        BB(j,i)   =  1/dt;
                        BB(j,i+1) =  D(cnt+1,i)/Qnew(cnt+1,i)*(Qnew(cnt+1,i+1)-Qnew(cnt+1,i-1))/(4*dx*dx);
                        BB(j,i-1) =  -D(cnt+1,i)/Qnew(cnt+1,i)*(Qnew(cnt+1,i+1)-Qnew(cnt+1,i-1))/(4*dx*dx);
...
%             else    
%                 Cnew(j,i) = Cmod(j,i)+ dtmod2(j,i)*(D(j,i)/Qnew(j,i))...
%                     *((1/(4*dx*dx))*(Qnew(j+1,i+2)-Qnew(j+1,i))*(Ccal(j+1,i+2)-Ccal(j+1,i))...
%                     + (1/(4*dy*dy))*(Qnew(j+2,i+1)-Qnew(j,i+1))*(Ccal(j+2,i+1)-Ccal(j,i+1)));
%             end
%         end
%     end
%     Cnew(1,1)=c0;
